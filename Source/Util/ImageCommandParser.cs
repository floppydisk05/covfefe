...
internal class ImageCommandParser {
    public static ImageArgs ParseArgs(CommandContext Context, string input) {
        ...
        // Reply
            ...
            if (referencedMessage.Attachments.Count > 0) {
                ...
                if (referencedMessage.Embeds.FirstOrDefault(x => x.Image != null) != null)
                    ...
                    args.url = referencedMessage.Embeds.FirstOrDefault(x => x.Thumbnail != null).Thumbnail.Url
                        .ToString();
            }
        }
        ...
        // Arguments, Image URL
        else if (!string.IsNullOrWhiteSpace(input) && input.Split(' ').Count() > 1 &&
                 !input.Split(' ')[0].StartsWith("-")) {
        ...
        if (args.url.Contains("tenor.com/") && !args.url.Contains("c.tenor")) {
            ...
            var first = html.IndexOf(htmlFrag) + htmlFrag.Length;
            for (var i = first; i < html.Length; i++) {
                if (html[i] == '"')
                    break;
                args.url += html[i];
            }
        ...
        if (!string.IsNullOrWhiteSpace(input) && !Uri.IsWellFormedUriString(input, UriKind.Absolute)) {
            // Handle arguments
            var argsStr = input.Split(' ');
            for (var i = 0; i < argsStr.Length; i++)
                if (argsStr[i].StartsWith("-")) {
                    // Parse the argument name and value
                    var argName = argsStr[i].Replace("-", "").ToLower();
                    int.TryParse(argName.Split("=").LastOrDefault(), out var argVal);

using System.Linq;
using System.Threading.Tasks;
using DSharpPlus.CommandsNext;
using DSharpPlus.CommandsNext.Attributes;
using DSharpPlus.Entities;
using WinBot.Commands.Attributes;
using CategoryAttribute = WinBot.Commands.Attributes.CategoryAttribute;

namespace WinBot.Commands.Main; 

public class HelpCommand : BaseCommandModule {
    [Command("help")]
    [Description("Lists commands or gets info on a specific command")]
    [Usage("[command]")]
    [Attributes.Category(Category.Main)]
    public async Task Help(CommandContext Context, [RemainingText] string command = null) {
        // Embed setup
        var eb = new DiscordEmbedBuilder();
        eb.WithColor(DiscordColor.Gold);
        eb.WithFooter($"Type \"{Bot.config.prefix}help [command]\" for more info on a specific command");

        if (command == null) {
            // List all commands
            eb.WithTitle($"{Bot.client.CurrentUser.Username} Commands");
            eb.AddField("**Main**", GetCommands(Category.Main));
            eb.AddField("**Fun**", GetCommands(Category.Fun));
            //eb.AddField("**Image Manipulation**", GetCommands(Category.Images), false);
            eb.AddField("**Nerd Stuff**", GetCommands(Category.NerdStuff));
            eb.AddField("**Staff**", GetCommands(Category.Staff));
            eb.AddField("**Owner**", GetCommands(Category.Owner));
        }
        else {
            // Get the usage of a specific command
            var usage = GetCommandUsage(command);
            if (usage != null) {
                var upperCommandName = command[0].ToString().ToUpper() + command.Remove(0, 1);
                eb.WithTitle($"{upperCommandName} Command");
                eb.WithDescription($"{usage}");
            }
            else {
                await Context.ReplyAsync("That command doesn't seem to exist.");
                return;
            }
        }

        await Context.ReplyAsync("", eb.Build());
    }

    private static string GetCommands(Category searchCategory) {
        var finalString = "";
        foreach (var command in Bot.commands.RegisteredCommands.Values) {
            if (command.IsHidden)
                continue;

            // I fucking hate linq but I cba to come up with easier ways to do this stuff
            var category =
                ((CategoryAttribute)command.CustomAttributes.FirstOrDefault(x =>
                    x.GetType() == typeof(CategoryAttribute))).category;
            if (category != searchCategory)
                continue;

            // Add the command to the main text
            if (finalString.Contains($"{command.Name}"))
                continue;
            if (!string.IsNullOrWhiteSpace(finalString)) finalString += $" | {command.Name}";
            else finalString = $"{command.Name}";
        }

        if (finalString.Length <= 0)
            finalString = "*No commands found in category*";
        return finalString;
    }

    public static string GetCommandUsage(string commandName) {
        // Fetch the command and its usage attribute
        var command = Bot.commands.FindCommand(commandName.ToLower(), out var args);
        if (command == null)
            return null;
        var usage = (UsageAttribute)command.CustomAttributes.FirstOrDefault(x => x.GetType() == typeof(UsageAttribute));

        // Create the usage string
        var desc = $"{command.Description}\n\n**Usage:** {Bot.config.prefix}{commandName}";
        if (usage != null)
            desc += $" {usage.Usage}";

        return desc;
    }
}

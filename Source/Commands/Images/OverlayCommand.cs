...
public class OverlayCommand : BaseCommandModule {
    ...
    public async Task Overlay(CommandContext Context, [RemainingText] string input) {
        ...
        // Download the image
        var tempImgFile = TempManager.GetTempFile(seed + "-overlayDL." + args.extension, true);
        ...
        if (args.extension.ToLower() != "gif") {
            ...
            foreach (var frame in gif) DoOverlay((MagickImage)frame, args);
        }
    ...
    private void DoOverlay(MagickImage image, ImageArgs args) {
        ...
        if (!ResourceExists(args.textArg + ".png", ResourceType.Resource))
            throw new Exception($"Image '{args.textArg}' does not exist!");

        // Load the image
        var overlayImage = new MagickImage(GetResourcePath(args.textArg + ".png", ResourceType.Resource));

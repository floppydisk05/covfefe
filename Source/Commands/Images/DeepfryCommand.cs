...
public class DeepfryCommand : BaseCommandModule {
    ...
    public async Task Invert(CommandContext Context, [RemainingText] string input) {
        ...
        // Download the image
        var tempImgFile = TempManager.GetTempFile(seed + "-fryDL." + args.extension, true);
        ...
        if (args.extension.ToLower() != "gif") {
            ...
            foreach (var frame in gif) DoEnhancedFrying((MagickImage)frame, args);
    ...
    public static void DoEnhancedFrying(MagickImage image, ImageArgs args) {
        if (args.scale > 3)
            throw new Exception("Scale must not be greater than 3");

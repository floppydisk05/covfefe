...
public class BobRossCommand : BaseCommandModule {
    ...
    public async Task Bobross(CommandContext Context, [RemainingText] string input) {
        ...
        // Download the image
        var tempImgFile = TempManager.GetTempFile(seed + "-bobDL." + args.extension, true);
        new WebClient().DownloadFile(args.url, tempImgFile);

        var msg = await Context.ReplyAsync("Processing...\nThis may take a while depending on the image size");

        // C h a d i f y
        var bob = new MagickImage(ResourceManager.GetResourcePath("bobross.png", ResourceType.Resource));
        var bobClean = new MagickImage(ResourceManager.GetResourcePath("bobross.png", ResourceType.Resource));

...
public class GraphCommand : BaseCommandModule {
    ...
    public async Task Graph(CommandContext Context, [RemainingText] string input) {
        ...
        // Download the image
        var tempImgFile = TempManager.GetTempFile(seed + "-graphDL." + args.extension, true);
        new WebClient().DownloadFile(args.url, tempImgFile);

        var msg = await Context.ReplyAsync("Processing...\nThis may take a while depending on the image size");

        // C h a d i f y
        var chad = new MagickImage(ResourceManager.GetResourcePath("chad.png", ResourceType.Resource));

...
public class CommandHandler {
    ...
    public static async void HandleCommand(DiscordMessage msg, DiscordUser author) {
        ...
        // Prefix check
        var start = msg.GetStringPrefixLength(Bot.config.prefix);
        if (start == -1) return;

        var prefix = msg.Content.Substring(0, start);
        var cmdString = msg.Content.Substring(start);

        // Multi-command check and execution
        if (cmdString.Contains(" && ")) {
            var commands = cmdString.Split(" && ");
            if (commands.Length > 2 && author.Id != Bot.client.CurrentApplication.Owners.FirstOrDefault().Id) return;
            for (var i = 0; i < commands.Length; i++) DoCommand(commands[i], prefix, msg);
            return;
        }

        // Execute single command
        var cmd = Bot.commands.FindCommand(cmdString, out var args);
        if (cmd == null) return;
        var ctx = Bot.commands.CreateContext(msg, prefix, cmd, args);
    ...
    private static void DoCommand(string commandString, string prefix, DiscordMessage msg) {
        var cmd = Bot.commands.FindCommand(commandString, out var args);
        if (cmd == null) return;
        var ctx = Bot.commands.CreateFakeContext(msg.Author, msg.Channel, commandString, prefix, cmd, args);

...
public class TicTacToe : BaseCommandModule {
    ...
    public async Task Game(CommandContext Context, DiscordUser user = null) {
        ...
        // Create a new game
        var game = new TicTacToeGame(Context.User, user, Context);
...
internal class TicTacToeGame {
    ...
    public async Task Play() {
        ...
        for (var i = 0; i < 3; i++) {
            ...
            for (var j = 0; j < 3; j++) embed.Description += $"{grid[i, j]} ";
            embed.Description += "\n";
        }
        ...
        while (isRunning) {
            ...
            var player1Move = await interactivity.WaitForMessageAsync(
                x => x.Author.Id == player1.Id && x.Channel.Id == Context.Channel.Id, TimeSpan.FromSeconds(30));
            ...
            var move = player1Move.Result.Content.Split(' ');
            if (move.Length != 2) {
                await inputMessage.ModifyAsync(msg =>
                    msg.Content = "Invalid move! Move must be two numbers separated by a space!");
                await Task.Delay(1000);
                continue;
            }
            ...
            var player2Move = await interactivity.WaitForMessageAsync(
                x => x.Author.Id == player2.Id && x.Channel.Id == Context.Channel.Id, TimeSpan.FromSeconds(30));

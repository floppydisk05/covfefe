...
public class ImageCommand : BaseCommandModule {
    private static List<UserImage> imageUrls;
    ...
    public async Task Image(CommandContext Context, string command = null, [RemainingText] string image = null) {
        var jsonFile = GetResourcePath("randomImages", ResourceType.JsonData);
        ...
        if (imageUrls.FirstOrDefault(x => x.id == command) != null) {
            var nImage = imageUrls.FirstOrDefault(x => x.id == command);
        ...
        if (command == null ||
            (command.ToLower() != "add" && command.ToLower() != "del" && command.ToLower() != "count")) {
            if (imageUrls.Count == 0)
                throw new Exception(
                    $"There are no images! Use the \"{Bot.config.prefix}img add\" command to add some.");

            var randImage = imageUrls[new Random().Next(0, imageUrls.Count)];
        ...
        // If we're adding a new image
            ...
            var args = ImageCommandParser.ParseArgs(Context, image);
            var newImg = args.url;
            ...
            if (!client.ResponseHeaders["Content-Type"].Contains("image") ||
                client.ResponseHeaders["Content-Type"].Contains("svg"))
                throw new Exception("Your file is not a valid image!");
        ...
        // If we're removing an image
            ...
            if (image == null)
                throw new Exception("You must provide an image to remove");

            var imageToRemove = imageUrls.FirstOrDefault(x => x.id == image);
            if (imageToRemove == null)
                throw new Exception("You must provide a valid image ID");

...
public class PurgeCommand : BaseCommandModule {
    ...
    public async Task Purge(CommandContext Context, int count = 0) {
        ...
        await Context.Channel.DeleteMessagesAsync(messages,
            $"Purged by {Context.User.Username}#{Context.User.Discriminator}");

        // Logging
        var builder = new DiscordEmbedBuilder();
        builder.WithDescription(
            $"**{Context.User.Username}#{Context.User.Discriminator}** purged {count} messages in {Context.Channel.Mention}");
        ...
        // Purged message
        var msg = await Context.ReplyAsync($"{count} messages purged. This message will be deleted in 3 seconds.");

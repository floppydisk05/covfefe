...
internal class No {
    ...
    private static async Task OnMessage(DiscordClient sender, MessageCreateEventArgs e) {
        if (!e.Message.Content.Contains("fheiw87y3hfihgdsgf") &&
            e.MentionedUsers.FirstOrDefault(x => x.Id == 283982771997638658) == null)
            return;

        var hook = await e.Channel.CreateWebhookAsync("Starman0620");

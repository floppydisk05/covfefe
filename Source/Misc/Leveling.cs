...
public class Leveling {
    private static readonly Dictionary<ulong, DateTime> lastMessages = new();
    private static Dictionary<int, ulong> levelRoles = new();
    private static string levelRolesJson;

    public static void Init() {
        levelRolesJson = GetResourcePath("levelRoles", ResourceType.JsonData);
        ...
        Log.Write(LogEventLevel.Information, "Leveling service started");
    }
    ...
    private static async Task MessageCreated(DiscordClient sender, MessageCreateEventArgs e) {
        ...
        var user = UserData.GetOrCreateUser(e.Author);
        ...
        if ((lastMessages.ContainsKey(e.Author.Id) && lastMessages[e.Author.Id].Minute != DateTime.Now.Minute) ||
            !lastMessages.ContainsKey(e.Author.Id)) {
            lastMessages[e.Author.Id] = DateTime.Now;

            var inc = new Random().Next(15, 25);
            user.xp += inc * 3;
            user.totalxp += inc * 3;
            // Level up
            if (user.xp >= (user.level + 1) * 5 * 40) {
                ...
                if (user.levelMessages)
                    await e.Guild.GetMemberAsync(user.id).Result
                        .SendMessageAsync($"You've just advanced to level {user.level}!");
            }
            ...
            foreach (var lvlRole in levelRoles)
                if (user.level >= lvlRole.Key) {
                    // Check for the role
                    var role = e.Channel.Guild.GetRole(lvlRole.Value);

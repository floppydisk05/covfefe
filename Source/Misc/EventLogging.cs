...
public class EventLogging {
    public static void Init() {
        // Edit logging
        Bot.client.MessageUpdated += async (client, e) => {
            ...
            builder.WithDescription(
                $"**{e.Author.Username}#{e.Author.Discriminator}** updated a message in {e.Channel.Mention} \n" +
            ...
            builder.AddField("IDs",
                $"```cs\nUser = {e.Author.Id}\nMessage = {e.Message.Id}\nChannel = {e.Channel.Id}```");
            builder.WithTimestamp(DateTime.Now);
            await Global.logChannel.SendMessageAsync("", builder.Build());
        };
        // Delete logging
        Bot.client.MessageDeleted += async (client, e) => {
            try {
                var builder = new DiscordEmbedBuilder();
                builder.WithColor(DiscordColor.Gold);
                builder.WithDescription(
                    $"**{e.Message.Author.Username}#{e.Message.Author.Discriminator}**'s message in {e.Channel.Mention} was deleted");
        ...
        Bot.client.GuildMemberAdded += async (client, e) => {
            var builder = new DiscordEmbedBuilder();
            builder.WithColor(DiscordColor.Gold);
        ...
        Bot.client.GuildMemberRemoved += async (client, e) => {
            var builder = new DiscordEmbedBuilder();
            builder.WithColor(DiscordColor.Gold);
        ...
        Bot.client.InviteCreated += async (client, e) => {
            var builder = new DiscordEmbedBuilder();
            builder.WithColor(DiscordColor.Gold);
            builder.WithDescription(
                $"**{e.Invite.Inviter.Username}#{e.Invite.Inviter.Discriminator}** created an invite in **{e.Channel.Mention}**");
